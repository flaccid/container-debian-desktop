setup() {
    ...
    echo "autostart" > "$TEST_SKEL/.config/autostart/test.desktop"
    chmod 644 "$TEST_SKEL/.config/autostart/test.desktop"
    echo "vnc" > "$TEST_SKEL/.vnc/xstartup"
    chmod 644 "$TEST_SKEL/.vnc/xstartup"
}
...
@test "populate_home copies skeleton to home" {
    ...
    [ -f "$TEST_HOME/.config/autostart/test.desktop" ]
...
@test "populate_home makes autostart .desktop files executable" {
    ...
    [ -x "$TEST_HOME/.config/autostart/test.desktop" ]
}
...
@test "main logic: root, xfce4 exists, skips populate" {
...
@test "main logic: non-root, xfce4 exists, skips populate" {
    mkdir -p "$TEST_HOME/.config/xfce4"
    echo "existing" > "$TEST_HOME/.config/xfce4/settings.xml"
    export HOME="$TEST_HOME"
    run bash -c '
