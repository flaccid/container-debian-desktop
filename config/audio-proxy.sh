...
#
# Audio proxy — encodes raw PCM from PulseAudio to WebM/Opus (or AAC)
# for the noVNC audio plugin. Meant to sit behind websockify.
#
# Copyright (C) 2023 Mehrzad Asri
# Licensed under MPL 2.0
# Source: https://github.com/me-asri/noVNC-audio-plugin
#
...
proxy() {
	local pulse_port="$1" pulse_format="$2" pulse_sample_rate="$3" pulse_channels="$4"
	local secret_file="$5"
	local codec='opus' bitrate='96000' sample_rate='48000' secret

	local line
	while IFS= read -r line; do
		[ -z "${line}" ] && break
		case "${line}" in *':'*) ;; *) proto_error 'bad handshake' ;; esac
		local opt="$(echo "${line}" | cut -d ':' -f 1)"
	...
	[ -n "${secret_file}" ] && [ "${secret}" != "$(cat "${secret_file}")" ] && proto_error 'bad secret'

	case "${codec}" in
		'opus') opus_proxy "${pulse_port}" "${pulse_format}" "${pulse_sample_rate}" "${pulse_channels}" "${bitrate}" ;;
		'aac')  aac_proxy "${pulse_port}" "${pulse_format}" "${pulse_sample_rate}" "${pulse_channels}" "${bitrate}" "${sample_rate}" ;;
		*)      proto_error "invalid codec ${codec}" ;;
	esac
}

server() {
	local pulse_port="${PULSE_PORT}" pulse_format="${PULSE_FORMAT}" pulse_sample_rate="${PULSE_SAMPLE_RATE}" pulse_channels="${PULSE_CHANNELS}"
	local secret_file tcp_port tcp_bind="${TCP_BIND}" unix_socket
	...
	[ -z "${tcp_port}" ] && [ -z "${unix_socket}" ] && usage_error 'At least a listening TCP port or Unix socket is required'
	[ -n "${tcp_port}" ] && [ -n "${unix_socket}" ] && usage_error '-u and -l options are mutually exclusive'
...
if ! command -v socat >/dev/null 2>&1; then error 'socat not found. Is it installed?'; fi
if ! command -v gst-launch-1.0 >/dev/null 2>&1; then error 'GStreamer (gst-launch-1.0) not found. Is it installed?'; fi
