...
- **Screensaver idle timeout** defaults to **1 hour** (`/saver/timeout = 3600`). To change it, exec into the pod and run:
  ...
  - The X server's built-in screen saver is disabled via an XFCE autostart entry (`disable-x11-screensaver.desktop`) that runs `xset s off` after the session is fully initialised, so it doesn't interfere with xfce4-screensaver's timeout.
